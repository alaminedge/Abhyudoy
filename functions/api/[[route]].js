...
// Abhyudoy EdTech Platform — Complete API with Notifications, Tickets, Updates, Browse, Settings
...
async function makeUniqueSlug(db, table, slug, excludeId = null) {
  ...
  while (true) {
    const query = excludeId
      ? `SELECT COUNT(*) as cnt FROM ${table} WHERE slug = ? AND id != ?`
...
async function ensureTables(db) {
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS user_notifications (
    ...
    is_read INTEGER DEFAULT 0, read_at DATETIME,
...
async function handleUser(method, path, body, db, user) {
  ...
  // GET /api/user/enrollments (for browse page - check which courses user is enrolled in)
  if (method === 'GET' && path === '/enrollments') {
    const enrollments = await db.prepare(`
      SELECT course_id FROM memberships
  ...
  if (method === 'POST' && path === '/enrollments') {
    ...
    ).bind(user.id, course_id, expires_at, user.id).run();

    return json({ message: 'Enrolled successfully!', expires_at }, 201);
  }

  // GET /api/courses?status=published (browse catalog)
    ...
    const notifications = await db.prepare(`
      ...
      WHERE n.is_active = 1
        AND (n.target_type = 'all'
          OR (n.target_type = 'course' AND n.target_id IN (SELECT course_id FROM memberships WHERE user_id = ? AND is_active = 1))
  ...
  if (method === 'PUT' && path.match(/^\/notifications\/\d+\/read$/)) {
    ...
    await db.prepare(
      'INSERT OR REPLACE INTO user_notifications (user_id, notification_id, is_read, read_at) VALUES (?, ?, 1, datetime(\'now\'))'
    ).bind(user.id, notifId).run();
  ...
  if (method === 'GET' && path === '/profile-change-status') {
    const u = await db.prepare(
      'SELECT name_last_changed, email_last_changed, password_last_changed, institution FROM users WHERE id = ?'
    ).bind(user.id).first();
    return json({
  ...
  if (method === 'PUT' && path === '/profile') {
    ...
    // Check 90-day restriction for name
    if (name && name !== current.name) {
      if (current.name_last_changed) {
        ...
        if (new Date() < next) {
          ...
          return err(`Name can only be changed once every 90 days. ${days} days remaining.`, 403);
        }
      }
      await db.prepare('UPDATE users SET name = ?, name_last_changed = datetime(\'now\') WHERE id = ?')
        .bind(name.trim(), user.id).run();
    }

    // Check 90-day restriction for email
    if (email && email !== current.email) {
      if (current.email_last_changed) {
        const next = new Date(new Date(current.email_last_changed).getTime() + 90 * 86400000);
        if (new Date() < next) {
          const days = Math.ceil((next - new Date()) / 86400000);
          return err(`Email can only be changed once every 90 days. ${days} days remaining.`, 403);
        }
      }
      await db.prepare('UPDATE users SET email = ?, email_last_changed = datetime(\'now\') WHERE id = ?')
        .bind(email.toLowerCase().trim(), user.id).run();
    }

    if (institution !== undefined) {
      await db.prepare('UPDATE users SET institution = ? WHERE id = ?')
        .bind(institution.trim(), user.id).run();
    }
  ...
  if (method === 'PUT' && path === '/password') {
    ...
    if (u.password_last_changed) {
      ...
      if (new Date() < next) {
        const days = Math.ceil((next - new Date()) / 86400000);
        return err(`Password can only be changed once every 90 days. ${days} days remaining.`, 403);
      }
    }

    const newHash = await sha256(new_password);
    await db.prepare('UPDATE users SET password = ?, password_last_changed = datetime(\'now\') WHERE id = ?')
  ...
  if (method === 'GET' && path === '/latest-update') {
    const update = await db.prepare(
      'SELECT * FROM update_logs WHERE is_draft = 0 AND show_popup = 1 ORDER BY release_date DESC LIMIT 1'
    ).first();

    if (!update) return json({ update: null });

    const seen = await db.prepare(
      'SELECT * FROM user_seen_updates WHERE user_id = ? AND update_id = ?'
    ).bind(user.id, update.id).first();
  ...
  if (method === 'PUT' && path.match(/^\/updates\/\d+\/seen$/)) {
    ...
    await db.prepare(
      'INSERT OR IGNORE INTO user_seen_updates (user_id, update_id) VALUES (?, ?)'
    ).bind(user.id, updateId).run();
...
async function handleContact(method, path, body, db, user) {
  if (method === 'POST' && path === '/contact') {
    ...
    ).bind(user ? user.id : null, name.trim(), email.trim(), subject, message.trim()).run();
...
async function handleAdmin(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/courses') {
    ...
    const courseSlug = slug && slug.trim()
      ? await makeUniqueSlug(db, 'courses', generateSlug(slug))
      : await makeUniqueSlug(db, 'courses', generateSlug(title));

    const r = await db.prepare(
      `INSERT INTO courses (title, slug, description, thumbnail, is_active, show_in_browse, enrollment_type, subject_area, difficulty_level)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      title.trim(), courseSlug, description || '', thumbnail || null,
      is_active !== undefined ? is_active : 1,
      show_in_browse !== undefined ? show_in_browse : 0,
      enrollment_type || 'open', subject_area || null, difficulty_level || 'all'
    ).run();
  ...
  if (method === 'PUT' && path.match(/^\/courses\/\d+$/)) {
    ...
    if (title !== undefined) {
      u.push('title = ?'); v.push(title.trim());
      if (!slug) {
        const newSlug = await makeUniqueSlug(db, 'courses', generateSlug(title), courseId);
        u.push('slug = ?'); v.push(newSlug);
      }
    }
    if (slug !== undefined && slug.trim()) {
      const customSlug = await makeUniqueSlug(db, 'courses', generateSlug(slug), courseId);
      u.push('slug = ?'); v.push(customSlug);
    }
  ...
  if (method === 'POST' && path === '/chapters') {
    ...
    const r = await db.prepare('INSERT INTO chapters (paper_id, title, slug, sort_order) VALUES (?, ?, ?, ?)')
      .bind(paper_id, title, slug, sort_order || 0).run();
  ...
  if (method === 'POST' && path === '/lectures') {
    ...
    const r = await db.prepare(
      'INSERT INTO lectures (chapter_id, title, slug, yt_video_id, sort_order, description) VALUES (?, ?, ?, ?, ?, ?)'
    ).bind(chapter_id, title, slug, yt_video_id, sort_order || 0, description || '').run();
  ...
  if (method === 'POST' && path === '/notifications') {
    const { title, message, type, target_type, target_id, scheduled_at, expires_at } = body;
    if (!title || !message) return err('Title and message required');

    const r = await db.prepare(
      `INSERT INTO notifications (title, message, type, target_type, target_id, scheduled_at, expires_at, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      title.trim(), message.trim(), type || 'info',
      target_type || 'all', target_id || null,
      scheduled_at || null, expires_at || null, user.id
    ).run();

    // If publishing now (no schedule), distribute to users immediately
    if (!scheduled_at) {
      await distributeNotification(db, r.meta.last_row_id, target_type, target_id);
    }
  ...
  if (method === 'PUT' && path.match(/^\/support-tickets\/\d+\/resolve$/)) {
    ...
    await db.prepare(
      'UPDATE contact_messages SET is_resolved = 1, resolved_by = ?, resolved_at = datetime(\'now\') WHERE id = ?'
  ...
  if (method === 'POST' && path === '/updates') {
    ...
    const r = await db.prepare(
      'INSERT INTO update_logs (version, title, changelog, type, release_date, show_popup, dismissible, is_draft) VALUES (?, ?, ?, ?, ?, ?, ?, 0)'
  ...
  if (method === 'GET' && path === '/users') {
    const r = await db.prepare(
      'SELECT id, name, email, role, is_blocked, is_approved, institution, name_last_changed, email_last_changed, password_last_changed, created_at FROM users ORDER BY created_at DESC'
    ).all();
    return json(r.results);
  }
  ...
  if (method === 'PUT' && path.match(/^\/users\/\d+\/override-restriction$/)) {
    ...
    await db.prepare(
      'UPDATE users SET name_last_changed = NULL, email_last_changed = NULL, password_last_changed = NULL WHERE id = ?'
    ).bind(userId).run();
...
// Helper: Distribute notification to target users
async function distributeNotification(db, notificationId, targetType, targetId) {
  if (targetType === 'all') {
    ...
    for (const u of users.results) {
      ...
      ).bind(u.id, notificationId).run();
    }
  } else if (targetType === 'user' && targetId) {
    await db.prepare(
      'INSERT OR IGNORE INTO user_notifications (user_id, notification_id) VALUES (?, ?)'
    ).bind(targetId, notificationId).run();
  } else if (targetType === 'course' && targetId) {
    const members = await db.prepare(
      'SELECT user_id FROM memberships WHERE course_id = ? AND is_active = 1 AND expires_at > datetime(\'now\')'
    ).bind(targetId).all();
    for (const m of members.results) {
      await db.prepare(
        'INSERT OR IGNORE INTO user_notifications (user_id, notification_id) VALUES (?, ?)'
      ).bind(m.user_id, notificationId).run();
    }
  }
}
...
export async function onRequest(context) {
  ...
  if (fullPath.startsWith('/courses')) {
    ...
    if (request.method === 'GET') {
      return handleUser('GET', '/courses', body, db, authUser);
    }
  }
  if (fullPath.startsWith('/contact')) return handleContact(request.method, fullPath, body, db, authUser);
