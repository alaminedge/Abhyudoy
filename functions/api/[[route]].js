// Abhyudoy EdTech Platform API — COMPLETE WORKING VERSION
...
async function ensureTables(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT DEFAULT 'user', is_blocked INTEGER DEFAULT 0, is_approved INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS device_registrations (id INTEGER PRIMARY KEY AUTOINCREMENT, device_fingerprint TEXT NOT NULL, user_id INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS courses (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT, thumbnail TEXT, is_active INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS subjects (id INTEGER PRIMARY KEY AUTOINCREMENT, course_id INTEGER NOT NULL, name TEXT NOT NULL, has_papers INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS papers (id INTEGER PRIMARY KEY AUTOINCREMENT, subject_id INTEGER NOT NULL, name TEXT NOT NULL, sort_order INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (subject_id) REFERENCES subjects(id) ON DELETE CASCADE)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS chapters (id INTEGER PRIMARY KEY AUTOINCREMENT, paper_id INTEGER NOT NULL, title TEXT NOT NULL, sort_order INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (paper_id) REFERENCES papers(id) ON DELETE CASCADE)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS lectures (id INTEGER PRIMARY KEY AUTOINCREMENT, chapter_id INTEGER NOT NULL, title TEXT NOT NULL, yt_video_id TEXT NOT NULL, sort_order INTEGER DEFAULT 0, description TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (chapter_id) REFERENCES chapters(id) ON DELETE CASCADE)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS lecture_pdfs (id INTEGER PRIMARY KEY AUTOINCREMENT, lecture_id INTEGER NOT NULL, title TEXT NOT NULL, pdf_url TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (lecture_id) REFERENCES lectures(id) ON DELETE CASCADE)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS resources (id INTEGER PRIMARY KEY AUTOINCREMENT, course_id INTEGER NOT NULL, title TEXT NOT NULL, pdf_url TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE)`).run();
  await db.prepare(`CREATE TABLE IF NOT EXISTS memberships (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, course_id INTEGER NOT NULL, expires_at DATETIME NOT NULL, is_active INTEGER DEFAULT 1, granted_by INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, course_id), FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE)`).run();
  
  const adminHash = await sha256('123admin');
  await db.prepare(`INSERT OR IGNORE INTO users (name, email, password, role, is_approved) VALUES ('Admin', 'cnct.nx@gmail.com', ?, 'admin', 1)`).bind(adminHash).run();
}
...
async function handleUser(method, path, body, db, user) {
  ...
  if (method === 'GET' && path === '/my-courses') {
    const courses = await db.prepare(`SELECT c.*, m.expires_at FROM memberships m JOIN courses c ON m.course_id = c.id WHERE m.user_id = ? AND m.is_active = 1 AND m.expires_at > datetime('now') AND c.is_active = 1 ORDER BY m.created_at DESC`).bind(user.id).all();
...
async function handleAdmin(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/courses') {
    ...
    const r = await db.prepare('INSERT INTO courses (title, description) VALUES (?, ?)').bind(title, description||'').run();
  ...
  if (method === 'DELETE' && path.match(/^\/courses\/\d+$/)) {
    ...
    await db.prepare('DELETE FROM resources WHERE course_id = ?').bind(id).run();
    const subs = await db.prepare('SELECT id FROM subjects WHERE course_id = ?').bind(id).all();
    for (const s of subs.results) {
      const paps = await db.prepare('SELECT id FROM papers WHERE subject_id = ?').bind(s.id).all();
      for (const p of paps.results) {
  ...
  if (method === 'GET' && path === '/all-subjects') {
    const r = await db.prepare('SELECT * FROM subjects ORDER BY course_id, sort_order ASC').all();
    return json(r.results);
  }
  if (method === 'POST' && path === '/subjects') {
    const { course_id, name, has_papers } = body;
    if (!course_id || !name) return err('Course and name required');
    const r = await db.prepare('INSERT INTO subjects (course_id, name, has_papers) VALUES (?, ?, ?)').bind(course_id, name, has_papers!==undefined?has_papers:1).run();
    if (!has_papers || has_papers === 0) {
      await db.prepare('INSERT INTO papers (subject_id, name, sort_order) VALUES (?, ?, 0)').bind(r.meta.last_row_id, 'Full Course').run();
    }
    return json({ id: r.meta.last_row_id, message: 'Subject created' }, 201);
  }
  if (method === 'DELETE' && path.match(/^\/subjects\/\d+$/)) {
    const id = parseInt(path.split('/')[2]);
    const paps = await db.prepare('SELECT id FROM papers WHERE subject_id = ?').bind(id).all();
    for (const p of paps.results) {
      const chs = await db.prepare('SELECT id FROM chapters WHERE paper_id = ?').bind(p.id).all();
      for (const c of chs.results) {
        await db.prepare('DELETE FROM lecture_pdfs WHERE lecture_id IN (SELECT id FROM lectures WHERE chapter_id = ?)').bind(c.id).run();
  ...
  if (method === 'GET' && path === '/all-papers') {
    const r = await db.prepare('SELECT * FROM papers ORDER BY subject_id, sort_order ASC').all();
    return json(r.results);
  }
  ...
  if (method === 'DELETE' && path.match(/^\/papers\/\d+$/)) {
    const id = parseInt(path.split('/')[2]);
    const chs = await db.prepare('SELECT id FROM chapters WHERE paper_id = ?').bind(id).all();
    for (const c of chs.results) {
      await db.prepare('DELETE FROM lecture_pdfs WHERE lecture_id IN (SELECT id FROM lectures WHERE chapter_id = ?)').bind(c.id).run();
      await db.prepare('DELETE FROM lectures WHERE chapter_id = ?').bind(c.id).run();
    }
  ...
  if (method === 'GET' && path === '/all-chapters') {
    const r = await db.prepare('SELECT * FROM chapters ORDER BY paper_id, sort_order ASC').all();
    return json(r.results);
  }
  if (method === 'POST' && path === '/chapters') {
    const { paper_id, title } = body;
  ...
  if (method === 'DELETE' && path.match(/^\/chapters\/\d+$/)) {
    const id = parseInt(path.split('/')[2]);
    await db.prepare('DELETE FROM lecture_pdfs WHERE lecture_id IN (SELECT id FROM lectures WHERE chapter_id = ?)').bind(id).run();
  ...
  if (method === 'GET' && path === '/all-lectures') {
    const r = await db.prepare(`SELECT l.*, ch.title as chapter_title FROM lectures l JOIN chapters ch ON l.chapter_id = ch.id ORDER BY l.created_at DESC`).all();
    return json(r.results);
  }
  if (method === 'POST' && path === '/lectures') {
    const { chapter_id, title, yt_video_id } = body;
    if (!chapter_id || !title || !yt_video_id) return err('Chapter, title, and YT ID required');
    const r = await db.prepare('INSERT INTO lectures (chapter_id, title, yt_video_id) VALUES (?, ?, ?)').bind(chapter_id, title, yt_video_id).run();
    return json({ id: r.meta.last_row_id, message: 'Lecture created' }, 201);
  }
  if (method === 'DELETE' && path.match(/^\/lectures\/\d+$/)) {
    const id = parseInt(path.split('/')[2]);
    await db.prepare('DELETE FROM lecture_pdfs WHERE lecture_id = ?').bind(id).run();
  ...
  if (method === 'GET' && path === '/users') {
    const r = await db.prepare('SELECT id, name, email, role, is_blocked, is_approved, created_at FROM users ORDER BY created_at DESC').all();
    return json(r.results);
  }
  ...
  if (method === 'PUT' && path.match(/^\/users\/\d+\/block$/)) {
    ...
    return json({ message: is_blocked?'Blocked':'Unblocked' });
  }
  if (method === 'DELETE' && path.match(/^\/users\/\d+$/)) {
    const id = parseInt(path.split('/')[2]);
    await db.prepare('DELETE FROM memberships WHERE user_id = ?').bind(id).run();
    await db.prepare('DELETE FROM device_registrations WHERE user_id = ?').bind(id).run();
  ...
  if (method === 'GET' && path === '/memberships') {
    const r = await db.prepare(`SELECT m.*, u.name as user_name, c.title as course_title FROM memberships m JOIN users u ON m.user_id = u.id JOIN courses c ON m.course_id = c.id ORDER BY m.created_at DESC`).all();
    return json(r.results);
  }
  ...
  if (method === 'PUT' && path.match(/^\/memberships\/\d+\/extend$/)) {
    const { days } = body;
    const id = parseInt(path.split('/')[2]);
    await db.prepare(`UPDATE memberships SET expires_at = datetime(expires_at, '+' || ? || ' days'), is_active = 1 WHERE id = ?`).bind(days, id).run();
    return json({ message: 'Extended' });
  }
