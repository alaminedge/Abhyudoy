...
async function handleUser(method, path, body, db, user, request) {
  ...
  if (method === 'GET' && path.startsWith('/course/') && !path.startsWith('/course-by-id/')) {
  ...
  if (method === 'GET' && path.startsWith('/lecture/') && !path.startsWith('/lecture-by-id/')) {
      ...
      WHERE n.is_active = 1
        AND (
          ...
          OR (n.target_type = 'course' AND n.target_id IN (
            SELECT course_id FROM memberships WHERE user_id = ? AND is_active = 1
          ))
  ...
  if (method === 'PUT' && path === '/profile') {
    ...
    if (name && name !== current.name) {
      ...
      await db.prepare(
        "UPDATE users SET name = ?, name_last_changed = datetime('now') WHERE id = ?"
    ...
    if (email && email !== current.email) {
      ...
      await db.prepare(
        "UPDATE users SET email = ?, email_last_changed = datetime('now') WHERE id = ?"
  ...
  if (method === 'PUT' && path === '/password') {
    ...
    await db.prepare(
      "UPDATE users SET password = ?, password_last_changed = datetime('now') WHERE id = ?"
...
// CONTACT
// ─────────────────────────────────────────────
async function handleContact(method, body, db, user) {
  if (method === 'POST') {
...
async function handleAdmin(method, path, body, db, user) {
  ...
  if (method === 'POST' && path === '/papers') {
    ...
    const r = await db.prepare(
      'INSERT INTO papers (subject_id, name, sort_order) VALUES (?, ?, ?)'
  ...
  if (method === 'POST' && path === '/chapters') {
    ...
    const r = await db.prepare(
      'INSERT INTO chapters (paper_id, title, slug, sort_order) VALUES (?, ?, ?, ?)'
  ...
  if (method === 'POST' && path === '/notifications') {
    ...
    const r = await db.prepare(`
      INSERT INTO notifications (title, message, type, target_type, target_id, action_url, action_label, scheduled_at, expires_at, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
...
// ─────────────────────────────────────────────
// NOTIFICATION DISTRIBUTION HELPER
// ─────────────────────────────────────────────
async function distributeNotification(db, notificationId, targetType, targetId) {
  if (targetType === 'all') {
    const users = await db.prepare(
      'SELECT id FROM users WHERE is_approved = 1 AND is_blocked = 0'
    ).all();
    ...
    const members = await db.prepare(`
      ...
      WHERE course_id = ? AND is_active = 1 AND expires_at > datetime('now')
    `).bind(targetId).all();
...
export async function onRequest(context) {
  ...
  // Browse courses (public-ish, but needs user context for enrollment status)
  if (fullPath.startsWith('/courses')) {
    if (request.method === 'GET') {
      return handleUser('GET', '/courses', body, db, authUser, request);
    }
  }

  // Contact form
  if (fullPath.startsWith('/contact')) {
    return handleContact(request.method, body, db, authUser);
  }
